...
`GT-P06-S33` 將改善／惡化共同起轉門檻由 `0.1` 改為 `0.3`，惡化每日重設仍維持 `0.3`。Sample A／B 的 S33 固定三年結果分別與正式 S31 Baseline 同為 `105.205／103.555`，兩份 `periods.csv` 均逐位元一致；趨勢與 S32→S33 遷移測試也通過。此結果完成資料與策略相容性驗證，但在正式 Baseline 採用流程完成前，權威 Baseline 仍是 v13、`T2/S31`。
