...
> **文件狀態：現行技術定義兼遷移歷史。** 本頁繼續管理 Grade 趨勢狀態與遷移，不列為封存；早期任務表保留歷史，正式版本與交易用途見[現行規則](現行回測規則.md)。
...
2026/09/04 起，UI 再以此已持久化的運行極值衍生探頂／探底前後期：改善運行高點達 `+0.911888` 或惡化運行低點達 `-0.911888` 為後期，未達為前期。該門檻是確認門檻 `0.611888` 加既有轉折幅度 `0.3`；前期圖示用 58% 色彩透明度，後期用原色。這不新增 raw phase 或 schema，不改狀態更新與交易規則，冷開機時也不重算歷史。
