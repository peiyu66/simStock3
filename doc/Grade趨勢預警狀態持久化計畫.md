...
| 2 | 改善預警 | 橙色實心右上箭頭 |
| 3 | 惡化預警 | 草黃色實心右下箭頭 |
| 4 | 改善確認（舊版未分段值） | 紅色實心右上箭頭 |
| 5 | 惡化確認（舊版未分段值） | 綠色實心右下箭頭 |
| 6 | 改善確認後退回 | 隱藏 |
| 7 | 惡化確認後退回 | 隱藏 |
| 8 | 改善確認探頂 | 紅色實心右上箭頭 |
| 9 | 改善確認拉回 | 紅色空心右下箭頭 |
...
`Trade.simFitTrendPhaseExtreme` 保存目前確認區段的運行高點或低點：改善確認保存高點，惡化確認保存低點，離開確認區段時為空。它讓每日更新直接延續前日區段，不必為每筆資料重新向前搜尋。

## 滾動與即時更新

每筆 Trade 只讀取前一交易日保存的階段、運行高低點及目前 `simFitTrend`，不向前搜尋。前一筆階段缺失時，視為狀態序列第一筆，直接依當日數值初始化；完整重播後若中途仍意外缺值，視為程式錯誤。

確認區段以運行極值切段。S33 的共同起轉門檻為 `0.3`：惡化確認從探底開始，當趨勢值嚴格高於運行低點 `0.3` 才轉為反彈；反彈後若出現新低點，或今日比前一正式交易日下降超過 `0.3`，當日重新標為探底，但不回改舊資料。改善確認從探頂開始，嚴格低於運行高點 `0.3` 才轉為拉回；拉回後出現新高點則重新標為探頂。改善方向不加入與惡化每日下降重設對稱的單日重設規則。

## S31 確認區段切割

S31 新增 `simFitTrendPhaseExtreme`，並把原改善／惡化確認各拆成兩個可交易時序階段。既有正式規則仍分別讀取改善確認或惡化確認的兩段聯集，因此這次資料結構調整不改變規則涵蓋範圍；未來候選才能一次移除其中一段，檢驗規則是否只在探底或反彈等特定階段有效。

`GT-P03-Z0-A／B` 已以 Baseline v12、`T2/S30` 完整重播，兩組主分、股群、窗口、交易事件與期末結果均為零差異。`StrategyFitTests` 與 `RecalculationTests` 也已通過分段轉換、同日 Yahoo 路徑獨立、`S30 → S31` 完整重播、極值寫入及人工操作重新驗證。程式與測試接線完成不等於正式切換；A／B／C／D S31 Baseline、全期間報告及 DecisionBase v6 尚未建立前，正式 Baseline 仍為 v12、`T2/S30`。
...
S32 先加入惡化反彈的單日重設：今日趨勢值比前一正式交易日下降超過 `0.3` 時，即使尚未跌破原運行低點，也重新進入探底並以當日值建立新的運行低點。這是處理反彈途中再次明顯轉弱，不改變改善方向。

`GT-P05` 以 Sample A／C 校準、Sample B 外部確認共同起轉門檻。起轉由 `0.1` 提高至 `0.3` 後，Sample B 惡化短促轉折由 24.5% 降至 12.3%，改善由 25.8% 降至 11.1%，兩方向的中位辨識延遲皆為 1 個交易日。依 Grade 拆解沒有形成單調且可重現的分界；改善若加入對稱的每日 `0.3` 重設，短促轉折反而升至 29.5%，因此不採 `byGrade` 或完全對稱重設。

`GT-P06-S33` 將改善／惡化共同起轉門檻由 `0.1` 改為 `0.3`，惡化每日重設仍維持 `0.3`。Sample A／B 的 S33 固定三年結果分別與正式 S31 Baseline 同為 `105.205／103.555`，兩份 `periods.csv` 均逐位元一致；趨勢與 S32→S33 遷移測試也通過。A／B 固定窗口、全期間與 DecisionBase v6 完成後，正式採用為 Baseline v14、`T2/S33`；C／D 未重播，仍只保留 S30 歷史證據。
