...
`simUpdate` 在買賣判斷前固定以前一交易日的 EMA／階段，加上今日價格與買賣前庫存，建立只存在記憶體的決策前趨勢；目前正式買賣規則尚不讀取它。買賣完成後，既有 `updateStrategyFitState` 仍從同一份前日狀態及買賣後資料重算並保存當日最終趨勢，供 UI 與次一交易日使用。兩次計算彼此不串接，決策前暫算不寫入 SwiftData，也不增加 observation；同一交易日 Yahoo 重算仍只使用前一交易日狀態與目前價格。
