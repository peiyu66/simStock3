...
enum InternalBacktestReport {
    ...
    static let isFormalHotKJBaselineV30 = !isFormalT3BaselineV21
    ...
    static let runID: String = {
        ...
        if isNineYearABProfile && candidate == .baseline && isFormalHotKJBaselineV30 {
            let window = isFullWindowStress ? "9y-fullstress" : "9y-fixed3y"
            return "baseline-\(sample.rawValue.lowercased())-v30-s42-hn13-kj-hot-t3s49-\(window)-600w-20260911"
        }
    ...
    static let referenceRunID: String = {
        ...
        if isNineYearABProfile {
            ...
            let window = isFullWindowStress ? "9y-fullstress" : "9y-fixed3y"
            if candidate == .baseline && isFormalHotKJBaselineV30 {
                return "baseline-\(lowerSample)-v29-s41-hp03a-pullback-market-t3s48-\(window)-600w-20260910"
            }
    ...
    static let reportTitle: String = {
        ...
        if isNineYearABProfile {
            ...
            if candidate == .baseline && isFormalHotKJBaselineV30 {
                return "Sample \(sample.rawValue) · T3/S49 S42 H-N13 空手 wow 探頂前期 K／J 過熱減分 \(window) Baseline"
            }
    ...
    static var baselineRuleVersion: String {
        if isFormalHotKJBaselineV30 {
            return "s42-hn13-kj-hot-20260911"
        }
    ...
    static let currentRuleChangeSummary: String = {
        if candidate == .baseline && isFormalHotKJBaselineV30 {
            return "H-N13：空手、決策 Grade 為 wow、個股探頂前期且 K／J 的 125 日標準分皆 >1.8 時，H 額外減 1；H 不成立才走原 L，其他票及上限不變。採用凍結 R2-H01-KJ，T3/S49／策略 S42；不新增 schema、不重算 tUpdate，重播 simUpdate 並重驗人工操作。固定窗口只有四個改善案例，效益集中友勁；九年交易與資金不變，既有長輪與超額加碼例外仍保留。"
        }
        if isNineYearABProfile && candidate == .baseline && isFormalMarketBaselineV22 {
    ...
    static let inputDirectoryName = isNineYearABProfile
        ? (isFormalT3BaselineV21 || isFormalMarketBaselineV22 || isFormalHotKJBaselineV30
    ...
    enum ReportError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .baselineConfigurationRequired:
                return "S49 正式 Baseline 必須使用九年凍結輸入設定；現有 v29 僅對應 T3/S48，不得以新版規則覆寫。"
    ...
    static func run(progress: (String) -> Void = { _ in }) throws -> Result {
        ...
        guard isNineYearABProfile && Technical.dataRuleVersion == "T3/S49" else {
        ...
        guard !CommandLine.arguments.contains(where: retiredFlags.contains) else {
            throw ReportError.invalidValues("舊版候選／Baseline 旗標已停用；K／J 限定 H 減分已正式採用為 T3/S49，歷史重現請使用原規則 commit。")
        }
        ...
        let decisionBaseID = [
            ...
            isFormalHotKJBaselineV30 ? "v16" : (isFormalMarketBaselineV22 ? "v8" : (isFormalT3BaselineV21 ? "v7" : "v6"))
        ].joined(separator: "-")
    ...
    private static func loadCrossSampleBaseline(from documents: URL) -> Baseline? {
        ...
        if isNineYearABProfile && isFormalHotKJBaselineV30 {
    ...
    private static func html(
        ...
        </style></head><body><main><div class="eyebrow">SIMSTOCK3 · SAMPLE \(sample.rawValue) BASELINE</div><h1>\(reportTitle)</h1><p class="sub">固定技術資料快照 · 起始本金 600 萬元\(isNineYearABProfile && candidate == .baseline && !isFormalHotKJBaselineV30 ? " · 九年初始基準，不與舊窗口直接比較" : " · 與 \(referenceRunID) 比較")</p>
