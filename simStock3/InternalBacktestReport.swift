...
enum InternalBacktestReport {
    ...
    static let isFormalPullbackMarketBaselineV29 = !isFormalT3BaselineV21
    ...
    static let candidate: Candidate = {
        ...
        if arguments.contains("--candidate-hp04-h9-s4") { return .hp04H9S4 }
    ...
    static let runID: String = {
        ...
        if isNineYearABProfile && candidate == .baseline && isFormalPullbackMarketBaselineV29 {
            let window = isFullWindowStress ? "9y-fullstress" : "9y-fixed3y"
            return "baseline-\(sample.rawValue.lowercased())-v29-s41-hp03a-pullback-market-t3s48-\(window)-600w-20260910"
        }
        ...
        case .hp04H9S2:
            return "hp04-h9-s2-\(sample.rawValue.lowercased())-prior-market-high9-fineplus-no-volume-vote-t3s42-9y-fixed3y-600w-20260907"
        case .hp04H9S3:
            return "hp04-h9-s3-\(sample.rawValue.lowercased())-prior-market-high9-fineplus-not-earlypeak-no-volume-vote-t3s42-9y-fixed3y-600w-20260907"
        case .hp04H9S4:
            let window = isFullWindowStress ? "9y-fullstress" : "9y-fixed3y"
            return "hp04-h9-s4-\(sample.rawValue.lowercased())-prior-market-high9-fineplus-not-earlypeak-nonneutral-no-volume-vote-t3s42-\(window)-600w-20260907"
        case .gwS01:
    ...
    static let referenceRunID: String = {
        ...
        if isNineYearABProfile {
            ...
            let window = isFullWindowStress ? "9y-fullstress" : "9y-fixed3y"
            if candidate == .baseline && isFormalPullbackMarketBaselineV29 {
                return "baseline-\(lowerSample)-v28-s39-hp02-flat-hp01-t3s46-\(window)-600w-20260909"
            }
    ...
    static let reportTitle: String = {
        ...
        if candidate == .hp04H9S4 {
            return "Sample \(sample.rawValue) · HP04-H9-S4 S3 且決策 Grade 趨勢非中性時取消 H-P04 加分"
        }
        if candidate == .hp04H9S2 {
            return "Sample \(sample.rawValue) · HP04-H9-S2 前一市場日九日高且 Grade ≥ fine 時取消 H-P04 加分"
        }
        if candidate == .hp04H9S1 {
            return "Sample \(sample.rawValue) · HP04-H9-S1 前一市場日最高等於九日最高時取消 H-P04 加分"
        }
        ...
        if isNineYearABProfile {
            ...
            if candidate == .baseline && isFormalPullbackMarketBaselineV29 {
                return "Sample \(sample.rawValue) · T3/S48 S41 H-P03a 個股及 Grade 拉回與前市場日探頂後期 \(window) Baseline"
            }
    ...
    static var baselineRuleVersion: String {
        if isFormalPullbackMarketBaselineV29 {
            return "s41-hp03a-pullback-market-20260910"
        }
    ...
    static let currentRuleChangeSummary: String = {
        if candidate == .baseline && isFormalPullbackMarketBaselineV29 {
            return "H-P03a 僅在有效 Grade、Grade 已暖機且改善拉回、個股價格拉回前／後期及前市場日探頂後期時取消原加分；空手／持股皆適用，H-P03b 共用上限不變。正式採用凍結 S3，T3/S48／策略 S41；不新增 schema、不重算 tUpdate，完整重播 simUpdate 並重驗人工操作。固定三年為採用主證據；A／台塑固定與京元九年反證、B 集中度仍保留。"
        }
        if isNineYearABProfile && candidate == .baseline && isFormalMarketBaselineV22 {
    ...
    static let currentRuleVersion: String = {
        ...
        case .hp04H9S4: return "s35-candidate-hp04-h9-s4"
        case .marketVoteNever, .marketVotePulseH, .marketVotePulseL,
             .marketVotePulseS, .marketVotePulseA:
    ...
    static let inputDirectoryName = isNineYearABProfile
        ? (isFormalT3BaselineV21 || isFormalMarketBaselineV22 || isFormalPullbackMarketBaselineV29
    ...
    enum ReportError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .baselineConfigurationRequired:
                return "S48 正式 Baseline 必須使用九年凍結輸入設定；現有 v28 僅對應 T3/S46，不得以新版規則覆寫。"
    ...
    static func run(progress: (String) -> Void = { _ in }) throws -> Result {
        ...
        guard isNineYearABProfile && Technical.dataRuleVersion == "T3/S48" else {
        ...
        guard !CommandLine.arguments.contains(where: retiredFlags.contains) else {
            throw ReportError.invalidValues("舊版候選／Baseline 旗標已停用；H-P03a 市場拉回限制已正式採用為 T3/S48，歷史重現請使用原規則 commit。")
        }
        ...
        if candidate == .hp04H9S1 || candidate == .hp04H9S2 || candidate == .hp04H9S3 || candidate == .hp04H9S4 {
            guard isNineYearABProfile, !isFullWindowStress || candidate == .hp04H9S4 else {
                throw ReportError.invalidValues("HP04-H9 僅支援現行固定三年窗口")
            }
            try InternalHP04High9Candidate.prepare()
        }
        ...
        let decisionBaseID = [
            ...
            isFormalPullbackMarketBaselineV29 ? "v15" : (isFormalMarketBaselineV22 ? "v8" : (isFormalT3BaselineV21 ? "v7" : "v6"))
        ].joined(separator: "-")
        ...
        let baseline = Baseline(
            ...
            marketInput: [
                "snapshotID": InternalMarketPricePathSellCandidate.sourceSnapshotID,
                ...
                "alignment": "strictly-before-decision-calendar-date",
        ...
        let manifest = Manifest(
            ...
            ruleVersion: currentRuleVersion,
            ruleCommit: ruleCommit,
            marketInput: baseline.marketInput,
            ruleChangeSummary: currentRuleChangeSummary,
            historyStart: historyStartText,
            through: dateText(through),
            moneyBaseWan: moneyBaseWan,
    ...
    private static func loadCrossSampleBaseline(from documents: URL) -> Baseline? {
        ...
        if isNineYearABProfile && isFormalPullbackMarketBaselineV29 {
    ...
    private static func html(
        ...
        </style></head><body><main><div class="eyebrow">SIMSTOCK3 · SAMPLE \(sample.rawValue) BASELINE</div><h1>\(reportTitle)</h1><p class="sub">固定技術資料快照 · 起始本金 600 萬元\(isNineYearABProfile && candidate == .baseline && !isFormalPullbackMarketBaselineV29 ? " · 九年初始基準，不與舊窗口直接比較" : " · 與 \(referenceRunID) 比較")</p>
