...
struct PriceBadge: View {
    ...
    private var hasFilledBackground: Bool {
        guard let action = trade.stock.p10Action,
              !action.isEmpty,
              let actionDate = trade.stock.p10Date else {
            return false
        }
        return actionDate == trade.date
    }

    private var usesCompactSymbols: Bool {
