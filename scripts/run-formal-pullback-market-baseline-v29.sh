...
readonly DERIVED_DATA="${SIMSTOCK_BASELINE_DERIVED_DATA:-${TMPDIR:-/tmp}/simStock3-formal-pullback-market-baseline-v29-derived}"
readonly BUNDLE_ID="com.peiyou.simStock3"
readonly RULE_VERSION="s41-hp03a-pullback-market-20260910"
...
step "Building one Debug App for all formal Baseline v29 runs"
xcodebuild build \
...
for sample in A B C D E; do
    ...
    decision_base_id="${sample_lower}-${profile_id}-${RULE_VERSION}-t3-s48-${RULE_COMMIT[1,12]}-fixed3y-20260722-v15"

    for window in fixed3y fullstress; do
        window_id="9y-${window}"
        run_id="baseline-${sample_lower}-v29-s41-hp03a-pullback-market-t3s48-${window_id}-600w-20260910"
        run_dir="${INTERNAL_ROOT}/Runs/${run_id}"
        complete_marker="${run_dir}/.complete"
        args=(
            --run-internal-backtest-report
            --nine-year-ab-baseline
            --formal-pullback-market-baseline-v29
            "${sample_flag[@]}"
        ...
        reference_id="baseline-${sample_lower}-v28-s39-hp02-flat-hp01-t3s46-${window_id}-600w-20260909"
        reference_source="${ROOT_DIR}/exports/backtest-reports/${reference_id}/baseline.json"
        [[ -f "$reference_source" ]] || fail "Missing v28 reference: ${reference_id}"
        ...
        [[ "$(json_raw "$manifest" dataRuleVersion)" == "T3/S48" ]] || fail "T/S mismatch: ${run_id}"
        ...
        if [[ "$window" == fixed3y || "$window" == fullstress ]]; then
            adopted="${ROOT_DIR}/exports/backtest-candidate-runs/hp03a-pullback-mkt-s3-${sample_lower}-candidate-t3s46-9y-${window}-600w-20260909/periods.csv"
            cmp -s "$adopted" "${run_dir}/periods.csv" || fail "Formal output differs from adopted HP03A-PULLBACK-MKT-S3: ${run_id}"
        fi
    ...
    step "Profiling Sample ${sample} DecisionBase v15"
    xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true
    ...
    python3 "${ROOT_DIR}/exports/audit-baseline-v29.py" "$sample"
done

step "Comparing formal Baseline v29 with adopted HP03A-PULLBACK-MKT-S3 evidence"
for sample in A B C D E; do
    sample_lower="${sample:l}"
    for window in fixed3y fullstress; do
        candidate="${ROOT_DIR}/exports/backtest-candidate-runs/hp03a-pullback-mkt-s3-${sample_lower}-candidate-t3s46-9y-${window}-600w-20260909/periods.csv"
        formal="${ROOT_DIR}/exports/backtest-reports/baseline-${sample_lower}-v29-s41-hp03a-pullback-market-t3s48-9y-${window}-600w-20260910/periods.csv"
...
step "Formal Baseline v29 and DecisionBase v15 outputs complete"
