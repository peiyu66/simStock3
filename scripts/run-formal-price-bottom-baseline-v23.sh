...
readonly DERIVED_DATA="${SIMSTOCK_BASELINE_DERIVED_DATA:-${TMPDIR:-/tmp}/simStock3-formal-price-bottom-baseline-derived}"
readonly BUNDLE_ID="com.peiyou.simStock3"
readonly RULE_VERSION="s34-sp09-price-bottom-early-sell-20260905"
...
step "Building one Debug App for all formal Baseline v23 runs"
xcodebuild build \
...
for sample in A B C D E; do
    ...
    decision_base_id="${sample_lower}-${profile_id}-${RULE_VERSION}-t3-s41-${RULE_COMMIT[1,12]}-fixed3y-20260722-v9"

    for window in fixed3y fullstress; do
        window_id="9y-${window}"
        run_id="baseline-${sample_lower}-v23-s34-sp09-price-bottom-early-sell-t3s41-${window_id}-600w-20260905"
        run_dir="${INTERNAL_ROOT}/Runs/${run_id}"
        complete_marker="${run_dir}/.complete"
        args=(
            --run-internal-backtest-report
            --nine-year-ab-baseline
            --formal-price-bottom-baseline-v23
            "${sample_flag[@]}"
        ...
        [[ "$(json_raw "$manifest" dataRuleVersion)" == "T3/S41" ]] || fail "T/S mismatch: ${run_id}"
        ...
        adopted="${ROOT_DIR}/exports/backtest-candidate-runs/rp-s03-${sample_lower}-price-seeking-bottom-early-sell-plus1-t3s40-9y-${window}-600w-20260904/periods.csv"
        cmp -s "$adopted" "${run_dir}/periods.csv" || fail "Formal output differs from adopted RP-S03: ${run_id}"
    ...
    step "Profiling Sample ${sample} DecisionBase v9"
    xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true
...
step "Comparing formal Baseline v23 with adopted RP-S03 evidence"
for sample in A B C D E; do
    sample_lower="${sample:l}"
    for window in fixed3y fullstress; do
        candidate="${ROOT_DIR}/exports/backtest-candidate-runs/rp-s03-${sample_lower}-price-seeking-bottom-early-sell-plus1-t3s40-9y-${window}-600w-20260904/periods.csv"
        formal="${ROOT_DIR}/exports/backtest-reports/baseline-${sample_lower}-v23-s34-sp09-price-bottom-early-sell-t3s41-9y-${window}-600w-20260905/periods.csv"
...
step "Formal Baseline v23 and DecisionBase v9 outputs complete"
