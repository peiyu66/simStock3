...
readonly DERIVED_DATA="${SIMSTOCK_BASELINE_DERIVED_DATA:-${TMPDIR:-/tmp}/simStock3-formal-hot-kj-baseline-v30-derived}"
readonly BUNDLE_ID="com.peiyou.simStock3"
readonly RULE_VERSION="s42-hn13-kj-hot-20260911"
...
step "Building one Debug App for all formal Baseline v30 runs"
xcodebuild build \
...
for sample in A B C D E; do
    ...
    decision_base_id="${sample_lower}-${profile_id}-${RULE_VERSION}-t3-s49-${RULE_COMMIT[1,12]}-fixed3y-20260722-v16"

    for window in fixed3y fullstress; do
        candidate_source="source"
        [[ "$window" == fullstress ]] && candidate_source="fullstress-20260911/source"
        window_id="9y-${window}"
        run_id="baseline-${sample_lower}-v30-s42-hn13-kj-hot-t3s49-${window_id}-600w-20260911"
        ...
        reference_id="baseline-${sample_lower}-v29-s41-hp03a-pullback-market-t3s48-${window_id}-600w-20260910"
        reference_source="${ROOT_DIR}/exports/backtest-reports/${reference_id}/baseline.json"
        [[ -f "$reference_source" ]] || fail "Missing v29 reference: ${reference_id}"
        ...
        [[ "$(json_raw "$manifest" dataRuleVersion)" == "T3/S49" ]] || fail "T/S mismatch: ${run_id}"
        ...
        if [[ "$window" == fixed3y || "$window" == fullstress ]]; then
            adopted="${ROOT_DIR}/exports/r2-h-kj-execution-20260910/${candidate_source}/exports/backtest-candidate-runs/r2-h01-kj-${sample_lower}-v29-t3s48-${window}-600w-20260910/periods.csv"
            cmp -s "$adopted" "${run_dir}/periods.csv" || fail "Formal output differs from adopted R2-H01-KJ: ${run_id}"
        fi
    ...
    step "Profiling Sample ${sample} DecisionBase v16"
    xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true
    ...
    python3 "${ROOT_DIR}/tools/audit_baseline_v30.py" "$sample"
done

step "Comparing formal Baseline v30 with adopted R2-H01-KJ evidence"
for sample in A B C D E; do
    sample_lower="${sample:l}"
    for window in fixed3y fullstress; do
        candidate_source="source"
        [[ "$window" == fullstress ]] && candidate_source="fullstress-20260911/source"
        candidate="${ROOT_DIR}/exports/r2-h-kj-execution-20260910/${candidate_source}/exports/backtest-candidate-runs/r2-h01-kj-${sample_lower}-v29-t3s48-${window}-600w-20260910/periods.csv"
        formal="${ROOT_DIR}/exports/backtest-reports/baseline-${sample_lower}-v30-s42-hn13-kj-hot-t3s49-9y-${window}-600w-20260911/periods.csv"
...
step "Formal Baseline v30 and DecisionBase v16 outputs complete"
