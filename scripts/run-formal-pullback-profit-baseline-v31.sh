...
readonly DERIVED_DATA="${SIMSTOCK_BASELINE_DERIVED_DATA:-${TMPDIR:-/tmp}/simStock3-formal-pullback-profit-baseline-v31-derived}"
readonly BUNDLE_ID="com.peiyou.simStock3"
readonly RULE_VERSION="s43-st01c-pullback-profit-20260911"
...
build_stamp="${DERIVED_DATA}/s50-source.sha256"
...
step "Building one Debug App for all formal Baseline v31 runs"
xcodebuild build \
...
for sample in A B C D E; do
    ...
    decision_base_id="${sample_lower}-${profile_id}-${RULE_VERSION}-t3-s50-${RULE_COMMIT[1,12]}-fixed3y-20260722-v17"

    for window in fixed3y fullstress; do
        candidate_source="exit-path-m3-execution-20260911/source"
        [[ "$window" == fullstress ]] && candidate_source="exit-path-m3-full-execution-20260911/source"
        window_id="9y-${window}"
        run_id="baseline-${sample_lower}-v31-s43-st01c-pullback-profit-t3s50-${window_id}-600w-20260911"
        ...
        args=(
            ...
            --formal-pullback-profit-baseline-v31
            "${sample_flag[@]}"
        ...
        reference_id="baseline-${sample_lower}-v30-s42-hn13-kj-hot-t3s49-${window_id}-600w-20260911"
        reference_source="${ROOT_DIR}/exports/backtest-reports/${reference_id}/baseline.json"
        [[ -f "$reference_source" ]] || fail "Missing v30 reference: ${reference_id}"
        ...
        [[ "$(json_raw "$manifest" dataRuleVersion)" == "T3/S50" ]] || fail "T/S mismatch: ${run_id}"
        ...
        if [[ "$window" == fixed3y || "$window" == fullstress ]]; then
            adopted="${ROOT_DIR}/exports/${candidate_source}/exports/backtest-candidate-runs/exit-path-m3-${sample_lower}-v30-t3s49-${window}-600w-20260911/periods.csv"
            cmp -s "$adopted" "${run_dir}/periods.csv" || fail "Formal output differs from adopted EXIT-PATH-M3: ${run_id}"
        fi
    ...
    step "Profiling Sample ${sample} DecisionBase v17"
    xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true
    ...
    python3 "${ROOT_DIR}/tools/audit_baseline_v31.py" "$sample"
done

step "Comparing formal Baseline v31 with adopted EXIT-PATH-M3 evidence"
for sample in A B C D E; do
    sample_lower="${sample:l}"
    for window in fixed3y fullstress; do
        candidate_source="exit-path-m3-execution-20260911/source"
        [[ "$window" == fullstress ]] && candidate_source="exit-path-m3-full-execution-20260911/source"
        candidate="${ROOT_DIR}/exports/${candidate_source}/exports/backtest-candidate-runs/exit-path-m3-${sample_lower}-v30-t3s49-${window}-600w-20260911/periods.csv"
        formal="${ROOT_DIR}/exports/backtest-reports/baseline-${sample_lower}-v31-s43-st01c-pullback-profit-t3s50-9y-${window}-600w-20260911/periods.csv"
...
step "Formal Baseline v31 and DecisionBase v17 outputs complete"
