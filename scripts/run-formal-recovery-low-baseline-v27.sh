...
readonly DERIVED_DATA="${SIMSTOCK_BASELINE_DERIVED_DATA:-${TMPDIR:-/tmp}/simStock3-formal-recovery-low-baseline-v27-derived}"
readonly BUNDLE_ID="com.peiyou.simStock3"
readonly RULE_VERSION="s38-lp10-held-nonflat-high9-20260908"
...
step "Building one Debug App for all formal Baseline v27 runs"
xcodebuild build \
...
for sample in A B C D E; do
    ...
    decision_base_id="${sample_lower}-${profile_id}-${RULE_VERSION}-t3-s45-${RULE_COMMIT[1,12]}-fixed3y-20260722-v13"

    for window in fixed3y fullstress; do
        window_id="9y-${window}"
        run_id="baseline-${sample_lower}-v27-s38-lp10-held-nonflat-high9-t3s45-${window_id}-600w-20260908"
        run_dir="${INTERNAL_ROOT}/Runs/${run_id}"
        complete_marker="${run_dir}/.complete"
        args=(
            --run-internal-backtest-report
            --nine-year-ab-baseline
            --formal-recovery-low-baseline-v27
            "${sample_flag[@]}"
        ...
        reference_id="baseline-${sample_lower}-v26-s37-lp03-flat-low-t3s44-${window_id}-600w-20260908"
        reference_source="${ROOT_DIR}/exports/backtest-reports/${reference_id}/baseline.json"
        [[ -f "$reference_source" ]] || fail "Missing v26 reference: ${reference_id}"
        ...
        [[ "$(json_raw "$manifest" dataRuleVersion)" == "T3/S45" ]] || fail "T/S mismatch: ${run_id}"
        ...
        if [[ "$window" == fixed3y || "$window" == fullstress ]]; then
            adopted="${ROOT_DIR}/exports/backtest-candidate-runs/lp10-held-nonflat-h9-s5-${sample_lower}-recovery-held-low-nonflat-market-high9-t3s44-9y-${window}-600w-20260908/periods.csv"
            cmp -s "$adopted" "${run_dir}/periods.csv" || fail "Formal output differs from adopted LP10-HELD-NONFLAT-H9-S5: ${run_id}"
        fi
    ...
    step "Profiling Sample ${sample} DecisionBase v13"
    xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true
...
step "Comparing formal Baseline v27 with adopted LP10-HELD-NONFLAT-H9-S5 evidence"
for sample in A B C D E; do
    sample_lower="${sample:l}"
    for window in fixed3y fullstress; do
        candidate="${ROOT_DIR}/exports/backtest-candidate-runs/lp10-held-nonflat-h9-s5-${sample_lower}-recovery-held-low-nonflat-market-high9-t3s44-9y-${window}-600w-20260908/periods.csv"
        formal="${ROOT_DIR}/exports/backtest-reports/baseline-${sample_lower}-v27-s38-lp10-held-nonflat-high9-t3s45-9y-${window}-600w-20260908/periods.csv"
...
step "Formal Baseline v27 and DecisionBase v13 outputs complete"
