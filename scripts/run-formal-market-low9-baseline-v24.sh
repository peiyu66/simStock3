...
readonly DERIVED_DATA="${SIMSTOCK_BASELINE_DERIVED_DATA:-${TMPDIR:-/tmp}/simStock3-formal-market-low9-baseline-derived}"
readonly BUNDLE_ID="com.peiyou.simStock3"
readonly RULE_VERSION="s35-sn01c-market-low9-sell-20260907"
...
step "Building one Debug App for all formal Baseline v24 runs"
xcodebuild build \
...
for sample in A B C D E; do
    ...
    decision_base_id="${sample_lower}-${profile_id}-${RULE_VERSION}-t3-s42-${RULE_COMMIT[1,12]}-fixed3y-20260722-v10"

    for window in fixed3y fullstress; do
        window_id="9y-${window}"
        run_id="baseline-${sample_lower}-v24-s35-sn01c-market-low9-sell-t3s42-${window_id}-600w-20260907"
        run_dir="${INTERNAL_ROOT}/Runs/${run_id}"
        complete_marker="${run_dir}/.complete"
        args=(
            --run-internal-backtest-report
            --nine-year-ab-baseline
            --formal-market-low9-baseline-v24
            "${sample_flag[@]}"
        ...
        reference_id="baseline-${sample_lower}-v23-s34-sp09-price-bottom-early-sell-t3s41-${window_id}-600w-20260905"
        reference_source="${ROOT_DIR}/exports/backtest-reports/${reference_id}/baseline.json"
        [[ -f "$reference_source" ]] || fail "Missing v23 reference: ${reference_id}"
        ...
        [[ "$(json_raw "$manifest" dataRuleVersion)" == "T3/S42" ]] || fail "T/S mismatch: ${run_id}"
        ...
        if [[ "$window" == fixed3y ]]; then
            adopted="${ROOT_DIR}/exports/backtest-candidate-runs/sn01-l9-s5-${sample_lower}-or-prior-low9-valid-nonwow-exclude-latepeak-fineplus-t3s41-9y-${window}-600w-20260907/periods.csv"
            cmp -s "$adopted" "${run_dir}/periods.csv" || fail "Formal output differs from adopted SN01-L9-S5: ${run_id}"
        fi
    ...
    step "Profiling Sample ${sample} DecisionBase v10"
    xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true
...
step "Comparing formal Baseline v24 with adopted SN01-L9-S5 evidence"
for sample in A B C D E; do
    sample_lower="${sample:l}"
    for window in fixed3y; do
        candidate="${ROOT_DIR}/exports/backtest-candidate-runs/sn01-l9-s5-${sample_lower}-or-prior-low9-valid-nonwow-exclude-latepeak-fineplus-t3s41-9y-${window}-600w-20260907/periods.csv"
        formal="${ROOT_DIR}/exports/backtest-reports/baseline-${sample_lower}-v24-s35-sn01c-market-low9-sell-t3s42-9y-${window}-600w-20260907/periods.csv"
...
step "Formal Baseline v24 and DecisionBase v10 outputs complete"
