...
readonly DERIVED_DATA="${SIMSTOCK_BASELINE_DERIVED_DATA:-${TMPDIR:-/tmp}/simStock3-formal-flat-low-baseline-v26-derived}"
readonly BUNDLE_ID="com.peiyou.simStock3"
readonly RULE_VERSION="s37-lp03-flat-low-20260908"
...
step "Building one Debug App for all formal Baseline v26 runs"
xcodebuild build \
...
for sample in A B C D E; do
    ...
    decision_base_id="${sample_lower}-${profile_id}-${RULE_VERSION}-t3-s44-${RULE_COMMIT[1,12]}-fixed3y-20260722-v12"

    for window in fixed3y fullstress; do
        window_id="9y-${window}"
        run_id="baseline-${sample_lower}-v26-s37-lp03-flat-low-t3s44-${window_id}-600w-20260908"
        run_dir="${INTERNAL_ROOT}/Runs/${run_id}"
        complete_marker="${run_dir}/.complete"
        args=(
            --run-internal-backtest-report
            --nine-year-ab-baseline
            --formal-flat-low-baseline-v26
            "${sample_flag[@]}"
        ...
        reference_id="baseline-${sample_lower}-v25-s36-hp04-market-high9-t3s43-${window_id}-600w-20260907"
        reference_source="${ROOT_DIR}/exports/backtest-reports/${reference_id}/baseline.json"
        [[ -f "$reference_source" ]] || fail "Missing v25 reference: ${reference_id}"
        ...
        [[ "$(json_raw "$manifest" dataRuleVersion)" == "T3/S44" ]] || fail "T/S mismatch: ${run_id}"
        ...
        if [[ "$window" == fixed3y || "$window" == fullstress ]]; then
            adopted="${ROOT_DIR}/exports/backtest-candidate-runs/lp03-flat-gt-s5-${sample_lower}-flat-grade-or-low-entry-t3s43-9y-${window}-600w-20260908/periods.csv"
            cmp -s "$adopted" "${run_dir}/periods.csv" || fail "Formal output differs from adopted LP03-FLAT-GT-S5: ${run_id}"
        fi
    ...
    step "Profiling Sample ${sample} DecisionBase v12"
    xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true
...
step "Comparing formal Baseline v26 with adopted LP03-FLAT-GT-S5 evidence"
for sample in A B C D E; do
    sample_lower="${sample:l}"
    for window in fixed3y fullstress; do
        candidate="${ROOT_DIR}/exports/backtest-candidate-runs/lp03-flat-gt-s5-${sample_lower}-flat-grade-or-low-entry-t3s43-9y-${window}-600w-20260908/periods.csv"
        formal="${ROOT_DIR}/exports/backtest-reports/baseline-${sample_lower}-v26-s37-lp03-flat-low-t3s44-9y-${window}-600w-20260908/periods.csv"
...
step "Formal Baseline v26 and DecisionBase v12 outputs complete"
