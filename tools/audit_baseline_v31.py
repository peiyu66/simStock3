"""Formal M3 S50 audit against frozen M3 and v30 risk context."""
import contextlib
import io
import json
import math
...
COMMIT = (ROOT / 'exports/baseline-v31-rule-commit.txt').read_text().strip()
RULE = 's43-st01c-pullback-profit-20260911'
...
def run_id(sample, version, window):
    suffix,day = ('s43-st01c-pullback-profit-t3s50','20260911') if version == 31 else ('s42-hn13-kj-hot-t3s49','20260911')
    return f'baseline-{sample.lower()}-v{version}-{suffix}-9y-{window}-600w-{day}'


def candidate_directory(sample, window):
    origin = 'exit-path-m3-execution-20260911' if window == 'fixed3y' else 'exit-path-m3-full-execution-20260911'
    return ROOT / 'exports' / origin / 'source/exports/backtest-candidate-runs' / f'exit-path-m3-{sample.lower()}-v30-t3s49-{window}-600w-20260911'
...
def audit(sample):
    ...
    bid = f'{sample.lower()}-{profile}-{RULE}-t3-s50-{COMMIT[:12]}-fixed3y-20260722-v17'
    ...
    assert bm['ruleCommit'] == COMMIT and bm['dataRuleVersion'] == 'T3/S50'
    ...
    for window in ('fixed3y', 'fullstress'):
        rid = run_id(sample, 31, window)
        directory = ROOT / 'exports/backtest-reports' / rid
        previous = ROOT / 'exports/backtest-reports' / run_id(sample, 30, window)
        ...
        for payload in (m, b):
            ...
            assert payload['ruleCommit'] == COMMIT and payload['dataRuleVersion'] == 'T3/S50'
        ...
        assert all(value in html for value in (COMMIT, 'T3/S50', RULE, run_id(sample, 30, window)))
        ...
        with contextlib.closing(connect(directory / 'browse.store')) as db:
            ...
            assert versions == [(3, 50, None, None)], (rid, versions)
    ...
    # Apply the frozen M3 delta to the immutable v30 DecisionBase, including gates.
    old_bid = f'{sample.lower()}-{profile}-s42-hn13-kj-hot-20260911-t3-s49-790d9666e9b5-fixed3y-20260722-v16'
    old_dir = ROOT / 'exports/backtest-decision-bases' / old_bid
    delta_dir = ROOT / 'exports/exit-path-m3-execution-20260911/source/exports/backtest-decision-deltas' / old_bid / 'EXIT-PATH-M3'
    expected, ids = {}, {}
    ...
    assert summary['candidateID'] == 'EXIT-PATH-M3' and summary['prestateMismatchCount'] == 0
    ...
    for window in ('fixed3y', 'fullstress'):
        fixed_dir = ROOT / 'exports/backtest-reports' / run_id(sample, 31, window)
        ...
        for name in names:
            ...
            with contextlib.closing(connect(fixed_dir / name)) as store:
                ...
                assert [tuple(r) for r in store.execute('SELECT DISTINCT ZTECHNICALSTATEVERSION,ZSIMULATIONSTATEVERSION,ZTECHNICALDIRTYFROM,ZSIMULATIONDIRTYFROM FROM ZSTOCK')] == [(3,50,None,None)]
...
if __name__ == '__main__':
    ...
    output = ROOT / ('exports/baseline-v31-verification-' + ''.join(samples) + '.json')
